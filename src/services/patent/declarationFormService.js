...
function generateSingleDeclaration(doc, inventor, title, applicationNumber) {
  let y = 40
  y = formHeader(doc, y)
...
export function generateAllDeclarations(app) {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'letter' })
  const meta = (app.metadata || {})
  const appNumber = meta.application_number || ''

  const inventors = app.inventors?.length ? app.inventors : (app.inventor_name ? [{
